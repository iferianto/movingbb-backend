...
class XDetectMobileBrowser extends CApplicationComponent
{
    ...
    /**
     * Call isMobileBrowser, store result in $isMobile.
    ...
    public function getIsMobile()
    {
        if (!isset($this->isMobile)) {
            ...
            if (isset($cookie)) {
                $this->isMobile = (bool)$cookie->value;
            } else {
                $this->isMobile = $this->isMobileBrowser();
                $this->setCookie(self::COOKIE_NAME_IS, $this->isMobile);
            }
        }
    ...
    public function getShowMobile()
    {
        if (!isset($this->showMobile)) {
            ...
            if (isset($cookie)) {
                ...
                $this->setCookie(self::COOKIE_NAME_SHOW, $this->showMobile);
            }
        }
    ...
    /**
     * Clears the cached result with the actual value
     */
     // Check if it can also be reset with:
     // ShowMobile = isMobileBrowser
     ...
     */
    private function setCookie($cookie_name, $value)
    {
        $cookie = new CHttpCookie($cookie_name, (int)$value);
